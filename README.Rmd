...
The goal of `stanova` is to provide a more relevant and interpretable `summary` for Bayesian models with categorical covariates and possibly interactions. The core function is `stanova()` which requires specifying which `rstanarm` function should be called through argument `model_fun` (e.g., `model_fun = glmer` calls `stan_glmer` and allows fitting Bayesian mixed models).
...
**`stanova` requires `rstanarm` version `2.21.2` which is not yet on CRAN, but must be installed from source**:
...
Also, at least version `1.4.7` of `emmeans` is needed which can be installed from CRAN:
``` r
