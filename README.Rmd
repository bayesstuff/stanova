...
`stanova` attaches the new contrasts to the model instead of to the data if `rstanarm` version `2.21.2`, which is not yet on CRAN, is installed from source:
...
Also, at least version `1.5.0` of `emmeans` is needed which can be installed from CRAN:
``` r
